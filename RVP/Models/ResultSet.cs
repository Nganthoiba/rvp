...
namespace RVP.Models
{
    public class ResultSet
    {
        ...
        public List<requested_mark> GetAllRequestResult(string search, string sortOrder, int start, int length, List<requested_mark> dtResult, List<string> columnFilters)
        { 
           return FilterAllRequestResult(search, dtResult, columnFilters).SortBy(sortOrder).Skip(start).Take(length).ToList();  
        }

        public int CountAllRequest(string search, List<requested_mark> dtResult, List<string> columnFilters)
        {
            return FilterAllRequestResult(search, dtResult, columnFilters).Count();
        }

        private IQueryable<requested_mark> FilterAllRequestResult(string search, List<requested_mark> dtResult, List<string> columnFilters)
        {
            IQueryable<requested_mark> results = dtResult.AsQueryable();
            ...
            results = results.Where(p => (search == null || (p.request_date != null && p.request_date.ToString("ddd, dd MMM yyyy, hh:mm tt").Contains(search)
                ...
                && (columnFilters[3] == null || (p.dob != null && p.dob.ToLower().Contains(columnFilters[3].ToLower())))
                && (columnFilters[4] == null || (p.txn_id != null && p.txn_id.ToLower().Contains(columnFilters[4].ToLower())))
                && (columnFilters[5] == null || (p.payment_status != null && p.payment_status.ToLower().Contains(status.ToLower())))
                );
